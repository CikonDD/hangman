...
/////////////// Math Riddle
function evenRandom() {
  let y = Math.ceil(Math.random() * 10 * 99);
  if (y % 10 !== 0) {
    for (let i = 1; i < 10; i++) {
      y = y + 1;
      if (y % 10 === 0) {
...
let x = evenevenRandom();
let resolver = x / 10;
let mathRiddle = {riddle:x,answer:[x,x*1.4,x*1.5,x*1.6]}
function solver(n) {
   var solution= [1.5*n,12.5*n,13.5*n,3.5*n]
    return solution
    
}
let solution = solver(resolver);
